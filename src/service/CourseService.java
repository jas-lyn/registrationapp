...
public class CourseService {
    ...
    public void enrollStudent(String courseId, Student student){           //enroll a student
        if (!courses.containsKey(courseId)){                               //if courseid is not found in enrolledStudents
            enrolledStudents.put(courseId, new ArrayList<>());             // put (add) the courseid to enrolledStudents, and instantiate a new array list
        }
        enrolledStudents.get(courseId).add(student);                        //thereafter, we add the student to the arraylist, after getting the course by its id
    }
    ...
    public void showEnrolledStudents(String courseId){                      //show all enrolled students
        if(enrolledStudents.containsKey(courseId)){                         //if key courseId exists in enrolledStudents
            ...
            students.forEach(student -> {                                   //iterate through the list and print each student info.
